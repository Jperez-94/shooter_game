...
const initNavigationEvents = () => {
    console.log("DEBUG: Detección de eventos de navegación");
    links.forEach((link) => {
...
const swipeTo = (getTo = "#menu_page") => {
    ...
    for (let i = 0; i < sections.length; i++) {
        let elem = "#" + sections[i].id;
        if (elem == getTo) {
            index = i;
            break;
        }
    }
    console.log("DEBUG: Animación de slide hacia " + sections[index].id);

    swiper.slideTo(index, 500, true);

    if (!swiper) {
        navigationErrHandler(
            `No has programado la funcionalidad de Swiper todavía!`
        );
    }
};
