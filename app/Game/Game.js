...
class Game {
    ...
    pauseOrResume() {
        if (this.paused) {
            this.timer = setInterval(() => this.update(), 50);
            GAME_UI.gameBoard.classList.remove('paused');
            this.paused = false;
        } else {
            clearInterval(this.timer);
            GAME_UI.gameBoard.classList.add('paused');
            this.paused = true;
        }
    }
    /**
    ...
    removeOpponent () {
        if (this.opponent) {
            GAME_UI.gameBoard.removeChild(this.opponent.image);
        }
    ...
    endGame () {
        ...
        if (this.player.lives > 0){
            let gameOver = new Entity(this, this.width / 2, "auto", this.width / 4, this.height / 4, 0, WIN_PICTURE);
            gameOver.render();
        } else{
    ...
    update () {
        if (!this.ended) {
            ...
            if (this.opponent === undefined) {
                console.log("Se crea un oponente");
        ...
        document.querySelector('.score .amount').innerHTML = this.score;
    }
